...
To create a new version of `litr`, we first install the latest release, then download `create-litr.Rmd` as it was at release and then use that installed version's `litr::render()` to create the new version:
...
*(Note: Until we make this repo public, one will need to add to the `install_github()` call `auth_token = gitcreds::gitcreds_get(use_cache=FALSE)$password)`)*
