...
`test_pkg::say_hello()` and `rhello::say_hello()`.

`rhello::say_hello()` and `test_pkg::say_hello()`.

`test-pkg::say_hello()` on a separate line.

`rhello::say_hello()` on a separate line.

test_pkg::say_hello() with no backticks.

rhello::say_hello() with no backticks.

say_hello() with no backticks.

We properly handle functions wrapped in a span tag with class "fu": <span class="fu">say_hello</span>()
