...
setup <- function(package_dir) {
  ...
  knitr::knit_hooks$set(document = function(x) {
    ...
    skipped_chunks <- which(sapply(original_code, function(x){
      echo_attr <- attr(x, "chunk_opts")$echo
      include_attr <- attr(x, "chunk_opts")$include
      if(!is.null(echo_attr) | !is.null(include_attr)){
        if(!is.null(echo_attr)){
          return(!echo_attr)
        } 
        if(!is.null(include_attr)){
          return(!include_attr)
        }
      } else{
        return(FALSE)
    ...
    # Next we remove the indices of skipped chunks and sort for good measure
    original_code_idx_fixed <- sort(setdiff(seq(length(original_code)), skipped_chunks))
