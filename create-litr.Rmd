...
This is an alternative way to get the special `litr` knitting behavior without relying on `litr::render()`.  The reason for this is that `rmarkdown::render()` allows for customizable behavior through the use of custom [output formats](https://bookdown.org/yihui/rmarkdown/new-formats.html).  This feels like the better way of doing things than requiring the user to use the function `litr::render()`.  That said, we should probably keep `litr::render()` for backward compatibility purposes.

Given a preexisting output format (e.g. `rmarkdown::html_document`), we would like to modify it to have `litr`-behavior -- i.e., to create a package as it is being rendered.  This next function takes a preexisting output format and "litr-ifies" it by making the following changes:

1. It modifies the `pre_knit()` function so that `litr::setup()` is called before the preexisting output format's `pre_knit()` function is called.  As the name suggests, this is a function that gets called before knitting.  

2. It modifies the `post_processor()` function so that the DESCRIPTION file gets marked with the version of `litr` used and with the `litr` hash (as already described [here](#hash)).
...
We use the above function to create some `litr` versions of common output formats.
...
However, for `litr` to recognize this as a function, we make it a bit longer with the `<- function(` format.
...
For .html documents, we'd like to add a bit more in the postprocessing step.  In particular, we'd like the special function and chunk hyperlinking behavior described [here](#render).
