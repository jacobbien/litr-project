...
Finally, we want to replace the ANSI escape sequences used by packages such as `testthat` and `devtools` with their HTML equivalents so our outputs are not illegible.
...
Since the above section uses the `fansi` package for handling ANSI escape sequences in HTML files, we include it in our package:
```{r}
