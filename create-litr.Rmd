...
### Combining grouping functions in .R files

By default, a separate .R file is created for each documented function. For example, if a function `foo` is defined in a code chunk, it will be written to the file `R/foo.R`. The is the default behavior the typical workflow for `litr`-based packages is centered around the Rmd file and users who want to read the R package code are recommended to read the generating Rmd file. However, users can control which .R file to write a function to by adding a `location` argument to the code chunk containing the function with the path to the preferred file name relative to `package_parent_dir` defined in the `knit-setup` chunk. For example, if we wanted to send the`foo` function to the file `R/bar.R`, we would add the code chunk argument `location="R/bar.R"` to the `foo` function code chunk. Then the next time the Rmd file is knitted, the `foo` function will be sent to `bar.R` file in the `R` folder of the package.
