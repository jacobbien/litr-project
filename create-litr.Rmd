...
With these steps outlined, we will show how this process in action on the following minimal Rmarkdown file:
...
preprocess_chunk_labels <- function(rmd_file){
    ...
    if(length(code_chunks) > 0){
      ...
      all_chunk_names <- code_chunks |> {\(x) purrr::map_chr(x, function(y){y$chunk_label})}()
...
The `add_chunk_label_hyperlinks()` function is similar to the `add_function_hyperlinks()` function except that it looks for comments within code chunks of the form `###"foo"###` and then whenever the chunk label `@@foo@@` is found it wraps a `a href="#foo"` tag so that it be a hyperlink to `foo`'s definition.
```{r}
...
### Testing `preprocess_chunk_labels()`

The purpose of these tests it to make sure that the `preprocess_chunk_labels()` function handles the following types of Rmd files properly:
...
testthat::test_that("preprocess_chunk_labels works as expected", {
  ...
  testthat::expect_error(preprocess_chunk_labels(missing_ref_chunk), "Unable to find the following chunk reference(s) in this Rmarkdown file: convert-c2f.", fixed=T)
})
```
### Testing `extract_rmd_code_chunks()`
