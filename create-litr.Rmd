...
6. Changes how [chunk references](https://bookdown.org/yihui/rmarkdown-cookbook/reuse-chunks.html) are handled. In particular, consider the following code chunk:
...
7. Define a `package_doc` engine which allows users to define package-level documentation.
...
#' R package directory.
#' * Sets up message and output hooks for converting control sequences to their HTML 
#' equivalents for better formatting the output of packages such as `testthat` and `usethis`.
...
setup <- function(package_dir) {
  ...
  # add message and output hooks for converting control sequences to HTML
  # equivalents
...
The code in this section used the `fs`, `knitr`, and `fansi` packages, so we import those:
...
The next function could have been defined in a single line: `litr_pdf_document <- litrify_output_format(rmarkdown::pdf_document)`

However, for `litr` to recognize this as a function, we make it a bit longer with the `<- function(` format and add documentation.
...
The function `add_function_hyperlinks()` looks for `foo` followed by `<- function(` and then wraps `foo` in a `span` tag with `id="foo"`; whenever `foo` is found it wraps a `a href="file.html#foo"` tag (where `file.html` is the file where `foo` is defined), so that it will be a hyperlink to `foo`'s definition.
...
##### `bookdown::gitbook()` output format
...
This first line makes it so that in RStudio when you press "Knit", it calls `bookdown::render_book()`, and the second line makes it so that the output format will be the special litr one.
